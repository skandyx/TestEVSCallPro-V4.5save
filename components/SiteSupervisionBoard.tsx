...
import type { AgentState, Site, AgentStatus } from '../types.ts';
...
interface SiteSupervisionBoardProps {
...
const SiteRow: React.FC<{
    site: Site | { id: 'no-site', name: string };
    agentsOnSite: AgentState[];
    onContactAgent: (agentId: string, agentName: string, message: string) => void;
}> = ({ site, agentsOnSite, onContactAgent }) => {
    const { t } = useI18n();
    const [isOpen, setIsOpen] = useState(true);

    const siteStats = useMemo(() => {
        return agentsOnSite.reduce((acc, agent) => {
        ...
        <div className="bg-slate-50 dark:bg-slate-900/50 rounded-lg border dark:border-slate-700">
            <button onClick={() => setIsOpen(!isOpen)} className="w-full flex items-center justify-between p-3 text-left">
                <div className="flex items-center gap-3">
                    ...
                    <h3 className="font-semibold text-lg text-slate-800 dark:text-slate-200">{site.name}</h3>
            ...
            {isOpen && (
                 <div className="border-t dark:border-slate-700">
                    <table className="min-w-full">
                        <thead className="sr-only"><tr><th>Agent</th><th>Status</th><th>Duration</th><th>Actions</th></tr></thead>
...
const SiteSupervisionBoard: React.FC<SiteSupervisionBoardProps> = ({ agentStates, sites, onContactAgent }) => {
    const { t } = useI18n();
    ...
    const sitesWithAgents = useMemo(() => {
        const agentsBySite = agentStates
            .reduce((acc, agent) => {
                ...
                acc[siteId].push(agent);
                return acc;
            }, {} as Record<string, AgentState[]>);

        return Object.keys(agentsBySite).map(siteId => {
            const site = sites.find(s => s.id === siteId) || { id: 'no-site', name: t('supervision.siteBoard.noSite') };
            return { site, agentsOnSite: agentsBySite[siteId] };
        }).sort((a, b) => a.site.name.localeCompare(b.site.name));
    }, [agentStates, sites, t]);

    return (
        <div className="space-y-4">
            {sitesWithAgents.length > 0 ? (
                sitesWithAgents.map(({ site, agentsOnSite }) => (
                    <SiteRow key={site.id} site={site} agentsOnSite={agentsOnSite} onContactAgent={onContactAgent} />
                ))
            ) : (
                <p className="text-center py-8 text-slate-500 dark:text-slate-400 italic">
                    {t('supervision.siteBoard.noActiveAgents')}
                </p>
            )}
        </div>
    );
...
export default SiteSupervisionBoard;